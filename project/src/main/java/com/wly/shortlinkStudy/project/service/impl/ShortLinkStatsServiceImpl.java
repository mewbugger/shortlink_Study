...
public class ShortLinkStatsServiceImpl implements ShortLinkStatsService {
    ...
    public ShortLinkStatsRespDTO oneShortLinkStats(ShortLinkStatsReqDTO requestParam) {
        // 根据短链接获取指定日期内基础监控数据，存到列表里
        List<LinkAccessStatsDO> listStatsByShortLink = linkAccessStatsMapper.listStatsByShortLink(requestParam);
        // 如果为空，则返回，前端根据null只显示响应空的图标界面
        ...
        // 基础访问详情
        // 生成一个日期范围内的每日短链接访问统计数据。如果某一天没有数据，那么会生成一个默认的统计数据（pv、uv、uip都为0）
        List<ShortLinkStatsAccessDailyRespDTO> daily = new ArrayList<>();
        // 使用DateUtil工具类生成一个日期范围的列表，然后将每个日期转换为字符串格式
        // 假设 requestParam.getStartDate() 返回 "2023-01-01"，requestParam.getEndDate() 返回 "2023-01-05"
        // 生成的 rangeDates 列表 ["2023-01-01", "2023-01-02", "2023-01-03", "2023-01-04", "2023-01-05"]
        ...
        int localeCnSum = listedLocaleByShortLink.stream()
                // 使用 mapToInt 方法将流中的每个元素映射为其 getCnt 属性的整数值。
    ...
    public IPage<ShortLinkStatsAccessRecordRespDTO> shortLinkStatsAccessRecord(ShortLinkStatsAccessRecordReqDTO requestParam) {
        ...
        if (CollUtil.isEmpty(linkAccessLogsDOIPage.getRecords())) {
            throw new ServiceException("当前时间段无访问记录");
        }
    ...
    @Override
    public IPage<ShortLinkGroupStatsAccessRecordRespDTO> groupShortLinkStatsAccessRecord(ShortLinkGroupStatsAccessRecordReqDTO requestParam) {
        LambdaQueryWrapper<LinkAccessLogsDO> queryWrapper = Wrappers.lambdaQuery(LinkAccessLogsDO.class)
                .eq(LinkAccessLogsDO::getGid, requestParam.getGid())
                .between(LinkAccessLogsDO::getCreateTime, requestParam.getStartDate(), requestParam.getEndDate())
                .eq(LinkAccessLogsDO::getDelFlag, 0)
                .orderByDesc(LinkAccessLogsDO::getCreateTime);
        IPage<LinkAccessLogsDO> linkAccessLogsDOIPage = linkAccessLogsMapper.selectPage(requestParam, queryWrapper);
        IPage<ShortLinkGroupStatsAccessRecordRespDTO> actualResult = linkAccessLogsDOIPage.convert(each -> BeanUtil.toBean(each, ShortLinkGroupStatsAccessRecordRespDTO.class));
        List<String> userAccessLogsList = actualResult.getRecords().stream()
                .map(ShortLinkGroupStatsAccessRecordRespDTO::getUser)
                .toList();
        List<Map<String, Object>> uvTypeList = linkAccessLogsMapper.selectGroupUvTypeByUsers(
                requestParam.getGid(),
                requestParam.getStartDate(),
